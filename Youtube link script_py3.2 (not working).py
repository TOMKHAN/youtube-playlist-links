...
more = 1
id_playlist = input("Enter youtube playlist id: ")
number_of_iteration  = int(input("How much video links: "))
...
while more <= number3:
     ...
     req = urlopen('http://gdata.youtube.com/feeds/api/playlists/'+ id_playlist + '?v=2&&start-index=' + str_start_index + '&max-results=50')
     #response = urllib2.urlopen(req)
     the_page = req.read()

     #writing page in .xml
     dat = open("web_content.xml","wb")
     dat.write(the_page)
     dat.close()
     ...
     new_string2 = new_string.replace("', 'rel': 'alternate', 'type': 'text/html'}","")
     f.close()
